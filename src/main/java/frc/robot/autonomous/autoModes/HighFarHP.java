...
public class HighFarHP extends SequentialCommandGroup {
    public HighFarHP(RobotContainer robotContainer) {
        PathPlannerTrajectory blueTrajectory = PathPlanner.loadPath("HighFarHPBlue", 3, 2);
        PathPlannerTrajectory startRedTrajectory = PathPlanner.loadPath("HighFarHP", 3, 2);
        PathPlannerTrajectory redTrajectory = PathPlannerTrajectory.transformTrajectoryForAlliance(startRedTrajectory, Alliance.Red);
        this.addCommands(
            new InstantCommand(() -> {
            ...
            new SequentialCommandGroup(  
                ...
                new ParallelDeadlineGroup(
                    new WaitCommand(2.0),
                    new ScoreAlignCommand(robotContainer.s_Swerve, 
                        DriverStation.getAlliance() == Alliance.Red? robotContainer.s_Visions : robotContainer.s_leftVisions, 
                        DriverStation.getAlliance() == Alliance.Red? Constants.xLeftCone : Constants.xRightCone, 
                        DriverStation.getAlliance() == Alliance.Red? Constants.yLeftCone : Constants.yRightCone)
                ),
