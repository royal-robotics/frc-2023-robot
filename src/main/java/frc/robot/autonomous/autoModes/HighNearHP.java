...
public class HighNearHP extends SequentialCommandGroup {
    public HighNearHP(RobotContainer robotContainer) {
        PathPlannerTrajectory blueTrajectory = PathPlanner.loadPath("HighNearHPBlue", 3, 2);
        PathPlannerTrajectory redPath = PathPlanner.loadPath("HighNearHP", 3, 2);
        PathPlannerTrajectory redTrajectory = PathPlannerTrajectory.transformTrajectoryForAlliance(redPath, Alliance.Red);
        this.addCommands(
            new InstantCommand(() -> {
            ...
            new SequentialCommandGroup(  
                ...
                new ParallelDeadlineGroup(
                    ...
                    new ScoreAlignCommand(robotContainer.s_Swerve, 
                        DriverStation.getAlliance() == Alliance.Blue? robotContainer.s_Visions : robotContainer.s_leftVisions, 
                        DriverStation.getAlliance() == Alliance.Blue?  0.805 : 0.805, 
                        DriverStation.getAlliance() == Alliance.Blue? -0.6 : Constants.yRightCone)
                ),
